...
private:
    ...
    void assertEqual(bool expected, bool actual, const std::string& testName) {
        if (expected == actual) {
            std::cout <<GREEN << "  [PASS] " << RESET << testName << " - ПРОЙДЕН" << std::endl;
            passed++;
        } else {
            std::cout << RED << "  [FAIL] " << RESET << testName << " - НЕ ПРОЙДЕН (Ожидалось: " << expected 
    ...
    void assertTrue(bool condition, const std::string& testName) {
        if (condition) {
            ...
            std::cout << RED << "  [FAIL] " << RESET << testName << " - НЕ ПРОЙДЕН" << std::endl;
            failed++;
        }
    }
